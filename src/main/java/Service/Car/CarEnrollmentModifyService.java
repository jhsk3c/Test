...
@Service
public class CarEnrollmentModifyService {
	...
	public void carEnrollmentModify(CarWriterCommand carWriterCommand, HttpServletRequest request, HttpSession session) {
		...
		carRepository.carModifyUpdate(car);
		 Long driveNum = carRepository.carOneDriveNumSelect(Long.parseLong(car.getMemNum()));
			System.out.println("업데이트할 드라이브넘 : " + driveNum);
			driveInfoDTO.setDriveNum(driveNum);
		 carRepository.driveInfoInUpdate(driveInfoDTO); 
	}
}
