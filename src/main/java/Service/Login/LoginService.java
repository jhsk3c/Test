...
public class LoginService { 
	...
	public Integer loginPro(LoginCommand loginCommand, HttpSession session, HttpServletResponse response) {
		...
		if(member == null) {
			...
			authInfo = 	new AuthInfo(member.getMemId(), member.getMemEmail(), member.getMemName(), member.getMemPw());	//로그인 정보 저장
	...
	public Integer comLoginPro(ComLoginCommand comloginCommand, HttpSession session, HttpServletResponse response) {
		...
		if(comMember == null) {
			...
			authInfo = 	new AuthInfo(comMember.getComId(), comMember.getContactEmail(), comMember.getComRepName(), comMember.getComPw());	//로그인 정보 저장
				session.setAttribute("authInfo", authInfo);
...
>>>>>>> branch 'master' of https://github.com/jhsk3c/HaeYum.git
